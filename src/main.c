...
static UpdateData *weather_data;
...
static void handle_tick(struct tm *tick_time, TimeUnits units_changed) {
  ...
  if (update_data->updated == 0 && weather_data->error == WEATHER_E_OK) {
    ...
    if (update_data->updated != last_updated_weather) {
...
static void init(void) {
  ...
  update_data = malloc(sizeof(WeatherData));
